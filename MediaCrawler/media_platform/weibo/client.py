...
class WeiboClient(ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[Response, Dict]:
        ...
        if ok_code == 0:  # response error
            # Search pagination uses this response to signal that the result set
            # is exhausted. Treat it as an empty page instead of retrying five
            # times and aborting the whole keyword batch.
            if data.get("msg") in {"这里还没有内容", "还没有人评论哦~快来抢沙发！"}:
