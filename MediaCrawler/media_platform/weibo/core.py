        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * weibo_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    if isinstance(last_error, DataFetchError) and "这里还没有内容" in str(last_error):
                        utils.logger.info(
                            f"[WeiboCrawler.search] no more results for keyword: {keyword}, page: {page}"
                        )
                        break
                    raise
                if not search_res.get("cards"):
                    utils.logger.info(
                        f"[WeiboCrawler.search] empty result page for keyword: {keyword}, page: {page}"
                    )
                    break
